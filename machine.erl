...
print_price()->
    ?VM ! {pprice,self()},
    receive
        {price,P} -> io:format("Price is : ~p ~n",[P])
    end.
    ...
    receive
        ...
        {insertCoins,Coi,CPid} ->
            ...
            if NewCoins < Price ->
                ...
                loop(NewState);
            true->
                X=lists:unzip(lists:ukeysort(1, List)),
                {BevList, _} = X,
                    ...
                    {givemebev,Bev,CPid} ->
                        ...
                        if M == false->
                            ...
                            true->
                                ...
                                io:format("Success order from : ~p ~nMoney returned: ~p ~n!",[CPid,ReturnCoins]),
...
select_beverage(B)->
    ?VM ! {givemebev,B,self()},
    receive
        {successorder,C,P} -> io:format("Success order of : ~p ~nMoney returned: ~p ~n!",[C,P]);
        {insufficient_founds,_}->insufficient_founds;
...
student(M)->
    ...
    R=insert_coins(M),
    if 
    R == insufficient_founds ->
        io:format("Sad Msg: no money!");
    length(R)==0->
        io:format("Sad Msg: no drinks!");
    true->
       B=select_beverage(lists:nth(rand:uniform(length(R)),R)),
